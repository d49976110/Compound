...
const coinBaseAddress = "0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984";
...
describe("Playground", function () {
    ...
    // describe("set up uniswap anchored view", async () => {
    //     async function deployOraclePriceDataFixture() {
    //         // get oraclePriceData
    //         const OraclePriceData = await ethers.getContractFactory(
    //             "OpenOraclePriceData"
    //         );
    ...
    //     it("owner put oracle price to oraclePriceData", async () => {
    //         const { oraclePriceData } = await loadFixture(
    //             deployOraclePriceDataFixture
    //         );
    //         let source = owner.address;
    //         let kind = "prices";
    //         let timestamp = 1666487095; // can't use Date.now, because we fork the mainnet, so the block.timestamp now is 1666587095, not the normal time
    //         let key = "owner";
    //         let value = 20n * 10n ** 6n; //Values stored in this contract should represent a USD price with 6 decimals precision
    //         let message = await ethers.utils.defaultAbiCoder.encode(
    ...
    //         // [aware!!] message types are string, uint64, string, uint64
    //         await oraclePriceData.put(message, signature);

    //         // let result = await oraclePriceData.getPrice(source, key);
    //         let result = await oraclePriceData.getPrice(source, key);
    ...
    //     it("set uniswapAnchoredView", async () => {
    //         const { oraclePriceData } = await loadFixture(
    //             deployOraclePriceDataFixture
    //         );
