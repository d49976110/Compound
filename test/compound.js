...
describe("Compound liquidate with change oracle", async () => {
    ...
    describe("Liquidate_change oracle price", async () => {
        ...
        it("liquidate", async () => {
            ...
            console.log(
                "pre addr1 balance of token a",
                await tokenA.balanceOf(addr1.address)
            );
            console.log(
                "pre addr1 balance of token b",
                await cTokenB.balanceOf(addr1.address)
            );
            console.log(
                "pre owner tokenB balance",
                await cTokenB.balanceOf(owner.address)
            );
            ...
            await cTokenA
                ...
                .liquidateBorrow(owner.address, repayAmount, cTokenB.address);
            console.log(
                "addr1 balance of token a",
                await tokenA.balanceOf(addr1.address)
            );
            console.log(
                "addr1 balance of token ctokenb",
                await cTokenB.balanceOf(addr1.address)
            );
            console.log(
                "owner tokenB balance",
                await cTokenB.balanceOf(owner.address)
            );
            console.log(
                "check liquidation",
                await comptroller.getAccountLiquidity(owner.address)
            );
        });
        it("liquidate again", async () => {
            let borrowBalance = await cTokenA.callStatic.borrowBalanceCurrent(
                owner.address
            );
