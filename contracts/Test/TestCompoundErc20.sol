...
contract TestCompoundErc20 {
  IERC20 public token;
  CErc20_test public cToken;

  event Log(string message, uint val);

  constructor(address _token, address _cToken) {
    token = IERC20(_token);
    cToken = CErc20_test(_cToken);
  }
  ...
  // borrow and repay //
  Comptroller_old public comptroller =
    Comptroller_old(0x3d9819210A31b4961b30EF54bE2aeD79B9c9Cd3B);
  ...
  function borrow(address _cTokenToBorrow, uint _decimals) external {
    ...
    require(errors[0] == 0, "Comptroller_old.enterMarkets failed.");
    ...
    require(CErc20_test(_cTokenToBorrow).borrow(amount) == 0, "borrow failed");
  }

  // borrowed balance (includes interest)
  // not view function
  function getBorrowedBalance(address _cTokenBorrowed) public returns (uint) {
    return CErc20_test(_cTokenBorrowed).borrowBalanceCurrent(address(this));
  }

  // borrow rate
  function getBorrowRatePerBlock(address _cTokenBorrowed) external view returns (uint) {
    // scaled up by 1e18
    return CErc20_test(_cTokenBorrowed).borrowRatePerBlock();
  }
  ...
  function repay(
    ...
    require(CErc20_test(_cTokenBorrowed).repayBorrow(_amount) == 0, "repay failed");
  }
}
