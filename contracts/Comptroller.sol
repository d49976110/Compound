...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            // tokensToDenom:代表該ctoken對於抵押狀態的面額
            ...
            // sumBorrowPlusEffects += oraclePrice * borrowBalance(underlying amount)
        ...
        if (vars.sumCollateral > vars.sumBorrowPlusEffects) {
            ...
            // 第三個參數為short fall : vars.sumBorrowPlusEffects - vars.sumCollateral
        ...
        returns (uint256, uint256)
    {
         ...
         */
        // 先計算出可得到返回的抵押Token數量 ＝ actualRepayAmount * liquidationIncentive(清算獎勵） * priceBorrowed，再除上priceCollateral
        // priceBorrowed，再除上priceCollateral => 兩個token的幣值比例換算，假設borrow的price = 10 collateral的價格為100，我協助清算了一顆borrow，等於我可以拿回10/100=0.1的collateral
    ...
    function _setCollateralFactor(CToken cToken, uint256 newCollateralFactorMantissa) external returns (uint256) {
        ...
        // If collateral factor != 0 and the oracle price price == 0, it will fail
